...
class Build:
    ...
    def render(self) -> Image:
        ...
        cam = Camera(self.CAM_EYE, self.CAM_LOOK_AT, self.UP_VECTOR, self.DISTANCE, self.HEIGHT, self.WIDTH)
        
        # Q[0][0] = C + (1/2 * s(n-1) * v) - (1/2 * s(m-1) * u)
        pixel_center_00 = cam.calc_pixel_center(self.PIXEL_SIZE)

        # computing the rays direction
        for i in range(self.HEIGHT): # C:\Users\pedro\Documents\NOVOdeltadelta\pog_refs\Ray-tracing\src\inputs\bolha-2.json
            for j in range(self.WIDTH):
                current_position = pixel_center_00 + self.PIXEL_SIZE * (j * cam.u - i * cam.v)
                ray_direction = current_position - cam.position  # alter
                ray_direction = self.normalize(ray_direction)
                ray = Ray(origin=cam.position, direction=ray_direction)
