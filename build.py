...
class Build:
    ...
    def render_scene(self) -> MyImage:
        screen = MyImage(self.HEIGHT, self.WIDTH,
                       self.BACKGROUND_COLOR)  # creating a screen
        ...
        def norm(x, y, z): return hypot(x, y, z)
        ...
        pixel_center_00 = screen_center + self.PIXEL_SIZE * \
            (((self.HEIGHT - 1)/2) * v - ((self.WIDTH - 1)/2) * u)
        progress = 1
        ...
        for i in range(self.HEIGHT):
            for j in range(self.WIDTH):
                ...
                color = []
                #+
                current_position = pixel_center_00 + \
                    self.PIXEL_SIZE * (j * u - i * v)
                ...
                #- color = [c * 255 / max(1, max(color)) for c in color]
                color = [int(c * 255 / max(1, max(color))) for c in color]

                #- color = color*255 / max(1, *color)
                if color:
                    max_color = max(color)
                    color = [int(c * 255 / max(1, max_color)) for c in color]
                else:
                    # Trate o caso em que a lista de cores está vazia
                    color = [0, 0, 0]  # Ou qualquer outra cor padrão que você desejar
                #+
                screen.set_pixel_color(i, j, color)
            print(
                f'progress: {progress * (self.WIDTH / self.PIXEL_SIZE)} pixels  |  {self.WIDTH * self.HEIGHT / self.PIXEL_SIZE} pixels', end='\r')
        return screen
    ...
    def cast(self, ray: Ray, max_depth: int) -> list:
        ...
        if intersection:
            # intersection point
            P = np.array(ray.origin + (t * ray.direction))
            ...
            if max_depth > 0:
                ...
                reflected_ray = Ray(origin=reflected_point,
                                    direction=reflected_vector)

                try:
                    if object.kt > 0:
                        refracted_vector = self.refract(
                            object, v, normal_vector)
                        refracted_point = P + (0.00001 * refracted_vector)
                        refracted_ray = Ray(
                            origin=refracted_point, direction=refracted_vector)
    ...
    def shade(self, _object, _P, _v, _n):
        ...
        cp += [_object.kd * _object.color[i] * light.intensity * np.dot(_n, to_light) if np.dot(_n, to_light) > 0 else 0 for i in range(3)]
        ...
        for light in self.lights:
            ...
            t, intersection, _ = self.find_intersection(
                shadow_ray)  # testing for each object

            if not intersection or np.dot(to_light, light.position - object_point) < t:
                # diffuse
                if np.dot(_n, to_light) > 0:
                    cp += np.multiply(_object.kd * _object.color,
                                      light.intensity * np.dot(_n, to_light))

                # specular
                if np.dot(_v, r) > 0:
                    cp += _object.ks * \
                        ((np.dot(_v, r) ** _object.exp) * light.intensity)
